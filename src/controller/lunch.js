...
class LunchController extends BaseController {
  ...
  async redeemLunchById(req, res) {
    ...
    if (!lunch) {
      return this.error(res, 'Lunch does not exist', 404);
    }

    if (lunch.redeemed) {
      return this.error(res, 'Lunch has already been redeemed', 403);
    }
    ...
    if (!organisation || !user) {
      return this.error(res, 'Error reedeming lunch', 404);
    }
    ...
    return this.success(res, 'Lunch redeemed successfully', 200);
  }

  async redeemAllUserLunches(req, res) {
    ...
    if (lunches.length == 0) {
      return this.error(
        res,
        'There are currently no lunch available to be redeemed',
        404
      );
    }
